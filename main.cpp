...
// Algoritmo minimax para decidir el mejor movimiento, ahora con poda alfa-beta
int minimax(std::vector<std::string> estado, int profundidad, bool esJugadorMax, int alfa, int beta) {
    ...
    if (esJugadorMax) {
        ...
        for (int i = 1; i <= 3; i++) {
            if (estado.size() >= i) {
                ...
                int valor = minimax(nuevoEstado, profundidad - 1, false, alfa, beta);
        ...
        for (int i = 1; i <= 3; i++) {
            if (estado.size() >= i) {
                std::vector<std::string> nuevoEstado = removerPalitos(estado, i);
                int valor = minimax(nuevoEstado, profundidad - 1, true, alfa, beta);
...
std::vector<std::string> movimientoIA(std::vector<std::string> palitos){
    ...
    for (int i = 1; i <= 3; i++) {
            ...
            int valor = minimax(nuevoEstado, 5, false, MIN, MAX);
            if (valor > mejorValor) {
                mejorValor = valor;
                mejorMovimiento = i;
            }
